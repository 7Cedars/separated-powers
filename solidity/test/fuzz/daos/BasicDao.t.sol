...
import { TestHelpers } from "../../../test/TestSetup.t.sol";
...
//                      Setup                      //
/////////////////////////////////////////////////////
abstract contract TestSetupBasicDao is Test, TestHelpers {
...
contract TestElectoralLaws is Test, TestHelpers, TestSetupBasicDao {
  ...
  function testFuzz_DelegateElect(uint256 numNominees, uint256 voteTokensRandomiser) public {
    ...
    for (uint256 i = 0; i < numNominees; i++) {
      ...
      basicDao.execute(
        laws[0], 
        lawCalldataNominate, 
        string.concat("Account nominates themself", Strings.toString(i)) 
      );
    }
    ...
    for (uint256 i = 0; i < numNominees; i++) {
      for (uint256 j = 0; j < numNominees; j++) {
        ...
        if (basicDao.hasRoleSince(nominee, 2) != 0 && basicDao.hasRoleSince(nominee2, 2) == 0) {
          ...
          assert (balanceNominee > balanceNominee2); // assert that nominee has more tokens than nominee2.
        }
        if (basicDao.hasRoleSince(nominee, 2) == 0 && basicDao.hasRoleSince(nominee2, 2) != 0) {
          uint256 balanceNominee = Erc20VotesMock(config.erc20VotesMock).balanceOf(nominee);
          uint256 balanceNominee2 = Erc20VotesMock(config.erc20VotesMock).balanceOf(nominee2);
          assert(balanceNominee < balanceNominee2); // assert that nominee has fewer tokens than nominee2.
        }
      }
    }
  }
  ...
  function testFuzz_VoteAndDirectSelect() public {
    // TODO
  }
...
// //   }
  ...
  function _distributeTokens(address erc20VoteMock, address[] memory accounts, uint256 randomiser) internal {
      ...
      for (uint256 i = 0; i < accounts.length; i++) {
          ...
          uint256 amount = currentRandomiser % 10_000;
