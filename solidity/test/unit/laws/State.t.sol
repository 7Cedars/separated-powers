...
contract AddressMappingTest is TestSetupState {
  ...
  function testSuccessfulAddingAddress() public {
    ...
    (
      ...
      ) = Law(addressesMapping).executeLaw(
        address(0), 
        lawCalldata, 
        keccak256("Adding an address")
        );
  ...
  function testAddingAddressRevertsIfAlreadyAdded() public {
    ...
    Law(addressesMapping).executeLaw(
        address(0), 
        lawCalldata, 
        keccak256("Adding an address first time")
        );
  ...
  function testSuccessfulRemovingAddress() public {
    ...
    Law(addressesMapping).executeLaw(
        address(0), 
        lawCalldataAdd, 
        keccak256("Adding an address to be removed")
    );
...
contract StringsArrayTest is TestSetupState {
  ...
  function testSuccessfulAddingString() public {    address stringsArray = laws[1];
    // prep
  ...
  function testSuccessfulRemovingString() public {
    ...
    vm.startPrank(address(daoMock));
    Law(stringsArray).executeLaw(
      address(0), 
      lawCalldataAdd, 
      keccak256("Adding a string to be removed"
    ));
  ...
  function testRemovingStringRevertsIfNoneAdded() public {
    ...
    Law(stringsArray).executeLaw(
      address(0), 
      lawCalldata, 
      keccak256("Removing a string not added"
    ));
  }
  ...
  function testRemovingStringRevertsIfStringNotFound() public {
    ...
    Law(stringsArray).executeLaw(
      address(0), 
      lawCalldataAdd, 
      keccak256("Adding a string not to be removed"
    ));
    ...
    Law(stringsArray).executeLaw(
      address(0), 
      lawCalldataRemove, 
      keccak256("Removing a string that does not exist"
    ));
  }
}
...
contract TokensArrayTest is TestSetupState {
  ...
  function testRemovingTokenRevertsIfNotAdded() public {
    ...
    Law(tokensArray).executeLaw(
        address(0), 
        lawCalldataRemove, 
        keccak256("Removing a non-existent token"
    ));
  ...
  function testRemovingTokenRevertsIfTokenNotFound() public {
    ...
    Law(tokensArray).executeLaw(
        address(0), 
        lawCalldataRemove, 
        keccak256("Removing a token that had not been added"
    ));
