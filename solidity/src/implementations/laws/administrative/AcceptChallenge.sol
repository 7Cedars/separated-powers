...
//         address agCoins_,
//         address Public_challengeRevoke // can take a address parentLaw param.
//     )
//         Law(
...
//         (, bytes memory revokeCalldata) = abi.decode(lawCalldata, (bytes32, bytes));
...
//         SeparatedPowersTypes.ProposalState stateChallenge = SeparatedPowers(payable(agDao)).state(parentProposalId);
//         if (stateChallenge != SeparatedPowersTypes.ProposalState.Completed) {
...
//         tar[1] = agDao;
//         val[1] = 0;
//         cal[1] = abi.encodeWithSelector(0xd2ab9970, 3, revokedMember, true); // = setRole(uint48 roleId, address account, bool access);
